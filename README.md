...
This project originated from research and development into generator setups and hydrological systems aimed at producing clean energy for **Non-Interconnected Zones (NIZ)**. While the broader initiative involves larger power units, the underlying control principles remain identical. As a result, we designed a compact, robust control architecture tailored to fit the precise requirements of rural micro-hydro applications.

---

## ✨ Key Features

- ⚙️ **Finite State Machine (FSM):** Controls operational modes (`OFF`, `SEARCH`, `TRACK`), ensuring safe transitions based on battery state and turbine speed.
- 🎛️ **PI Governor / PWM Control:** Regulates servovalve position using an adaptive PWM signal to adjust water flow according to generator RPM.
...
- ⏲️ **`rpm_meter.v`**: Hall sensor pulse counter and RPM range discriminator.
...
To run testbenches and generate waveform traces:
